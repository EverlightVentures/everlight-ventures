...
# Realistic NPC names - bots use these so the table feels lived-in
_NPC_NAMES = [
...
_used_npc_names: list[str] = []

def _pick_npc_name() -> str:
    """Pick a unique NPC name per session, cycling when exhausted."""
    global _used_npc_names
    available = [n for n in _NPC_NAMES if n not in _used_npc_names]
    if not available:
        _used_npc_names.clear()
        available = _NPC_NAMES[:]
    name = random.choice(available)
    _used_npc_names.append(name)
    return name

def _npc_starting_chips() -> float:
    """Randomized starting stack so bots don't all start at $100."""
    tiers = [50, 75, 100, 150, 200, 300, 500]
    weights = [10, 15, 30, 20, 15, 7, 3]
    return float(random.choices(tiers, weights=weights)[0])
...
def setup_game():
    ...
    for i in range(num_players):
        if i == 0:
            ...
            npc_name = _pick_npc_name()
            npc_chips = _npc_starting_chips()
            player_seats.append({'id': None, 'name': npc_name, 'balance': npc_chips, 'wager': 0, 'cards': [], '_is_npc': True})
