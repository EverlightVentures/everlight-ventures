...
import pytest
from kalshi_agent.execution.executor_polymarket_paper import PaperExecutor, PaperBetRequest


def test_paper_executor_does_not_import_wallet():
    """Critical: paper module must not reach the wallet module at all."""
    import kalshi_agent.execution.executor_polymarket_paper as paper_mod
    src = Path(paper_mod.__file__).read_text()
    assert "from kalshi_agent.execution.wallet" not in src
    assert "import kalshi_agent.execution.wallet" not in src
