...
class CoinbaseAPI:
    ...
    def move_portfolio_funds(self, amount: float, currency: str = "USDC",
        ...
        if not from_portfolio or not to_portfolio:
            logger.error("Could not identify source and destination portfolios")
            return None
    ...
    def place_cfm_order(
        ...
        order_config = {"base_size": str(rounded_size), "reduce_only": bool(reduce_only)}
        if price:
            # Round price to correct precision to avoid INVALID_PRICE_PRECISION error
        ...
        data = {
            "client_order_id": f"cfm_{side.lower()}_{int(time.time()*1000)}_{secrets.token_hex(4)}",
    ...
    def close_cfm_position(self, product_id: str) -> Optional[dict]:
        ...
        # Safety: place the reduce-only close first, then (optionally) cancel remaining orders only
    ...
    def get_price_precision(self, pair: str) -> int:
        ...
        Returns number of decimal places allowed for limit_price.
        """
        endpoint = f"/api/v3/brokerage/products/{pair}"
        result = self._request("GET", endpoint)
        if result:
            quote_increment = result.get("quote_increment", "0.01")
            # Count decimal places in quote_increment (e.g., "0.01" -> 2)
            if "." in quote_increment:
                return len(quote_increment.split(".")[1].rstrip("0")) or 2
        # Default to 2 for USD pairs
        return 2

    def _round_price(self, price: float, pair: str) -> float:
        """Round price to the appropriate precision for the pair"""
        precision = self.get_price_precision(pair)
