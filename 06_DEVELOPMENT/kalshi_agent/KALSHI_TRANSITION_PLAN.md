...
**K0b -- Fund Kalshi from the existing Polygon wallet (VERIFIED route 2026-06-02):**
  Kalshi accepts USDC deposits on **Polygon** directly via Zero Hash -> auto-converts
  to USD buying power (~30 min, no min). So the wallet's 116.51 USDC.e (Polygon)
  goes straight in -- NO bridge, NO off-ramp. Steps:
  1. Rich: Kalshi -> Add Funds -> Crypto -> USDC -> **Polygon** network. Copy the
     Zero Hash **deposit address** + note the EXACT token it expects.
  2. CAUTION: wallet holds USDC.e (bridged, 0x2791...); Zero Hash likely wants
     **native USDC** (0x3c49...). If so, I swap USDC.e -> native USDC on Polygon
     (Uniswap, ~pennies gas) before sending. (We have swap tooling from before.)
  3. **TEST-SEND FIRST:** I send $2-5, confirm it lands as USD in Kalshi, THEN send
     the rest. Deposit-address sends are IRREVERSIBLE -- the test is mandatory.
  4. I execute the on-chain swap+send (I hold the wallet key + web3). Gated on Rich
     pasting the verified Polygon deposit address.
  (Alt if crypto deposit ever fails: off-ramp USDC -> bank -> free ACH to Kalshi.)
