...
function deriveProductType(slug: string): string {
  ...
  if (slug.startsWith("hivemind-")) return "hivemind";
  if (slug.startsWith("intel-")) return "intelligence";
  return "unknown";
}

function isSubscription(slug: string): boolean {
  return slug === "arcade-vip-monthly" || slug === "ak-game-pass" || slug === "bj-game-pass" || slug === "master-pass" || slug.startsWith("hivemind-") || slug.startsWith("intel-");
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { slug, product_type: productType, ...safeMetadata },
    });
