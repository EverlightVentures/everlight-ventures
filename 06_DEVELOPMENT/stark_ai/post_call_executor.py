...
AGENT_SIGS = {
    ...
    "harrison": {
        ...
        "email": "harrison@everlightventures.io",
        "phone": "(888) 896-6772",
        "from": "Harrison Knox <harrison@everlightventures.io>",
    },
    "lucrex": {
        "name": "Lucrex",
...
<p>Would love to set up a quick call to walk through the specifics. You can <a href="https://jdqqmsmwmbsnlnstyavl.supabase.co/functions/v1/booking">book a time here</a> or just reply to this email.</p>""" % desc
    ...
    html = _branded_email(subject, body_html, agent_key=agent, to_name=to_name)
    ...
    print("  EMAIL -> %s (%s)" % (to_email, "OK" if resp.status_code in (200,201) else resp.text[:50]))
...
<p>Or just reply with a few times that work and I'll get us set up.</p>""" % desc
    ...
    html = _branded_email("Schedule a Call", body_html, agent_key="piper", to_name=to_name)

    resp = requests.post("https://api.resend.com/emails",
        headers={"Authorization": "Bearer %s" % RESEND_KEY, "Content-Type": "application/json"},
        json={
            "from": AGENT_SIGS["piper"]["from"],
            "to": [contact.get("email", RICH_EMAIL)],
            "reply_to": "piper@everlightventures.io",
            "subject": "Let's Get on the Calendar - %s" % desc,
            "html": html,
        }, timeout=15)
    print("  CALENDAR -> %s (%s)" % (contact.get("email"), "OK" if resp.status_code in (200,201) else "FAIL"))
    return resp.status_code in (200, 201)
...
<p>Please review at your convenience. If everything looks good, we can proceed immediately. If you have questions, just reply to this email or <a href="https://jdqqmsmwmbsnlnstyavl.supabase.co/functions/v1/booking">book a call with our team</a>.</p>""" % scope
            ...
            html = _branded_email("Finder Fee Agreement", body_html, agent_key="harrison", to_name=to_name)

            requests.post("https://api.resend.com/emails",
                headers={"Authorization": "Bearer %s" % RESEND_KEY, "Content-Type": "application/json"},
                json={
                    "from": AGENT_SIGS["harrison"]["from"],
                    "to": [contact.get("email", RICH_EMAIL)],
                    "reply_to": "harrison@everlightventures.io",
...
<p>If you have any questions about this invoice, reply to this email or contact our deal operations team.</p>""" % (desc, "{:,.2f}".format(amount), inv_url)
            ...
            html = _branded_email("Invoice - Everlight Ventures", body_html, agent_key="harrison", to_name=contact.get("name", "there").split()[0])
