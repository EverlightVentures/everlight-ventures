...
#   --notepad   : push the whole 05_PERSONAL/A_Personal_Notebook/NOTEPAD/ tree (transcripts library) to PC
...
do_notepad() {
  log "Push 05_PERSONAL/A_Personal_Notebook/NOTEPAD/ tree phone -> PC (--update, ~6 MB / 175 files)"
  ssh_pc "mkdir -p ${PC_WORKSPACE}/NOTEPAD" || true
  rsync -avz --update --human-readable "${RSYNC_EXCLUDES[@]}" \
    -e "${SSH_CMD}" \
    "${PHONE_WORKSPACE}/05_PERSONAL/A_Personal_Notebook/NOTEPAD/" \
    "${PC_USER}@${PC_HOST}:${PC_WORKSPACE}/05_PERSONAL/A_Personal_Notebook/NOTEPAD/" \
    2>&1 | tee -a "$LOG_FILE"
  # Drop an INGEST_QUEUE marker so the PC's claude session sees what's new.
  ssh_pc "cat > ${PC_WORKSPACE}/05_PERSONAL/A_Personal_Notebook/NOTEPAD/INGEST_QUEUE.md" <<EOF
