        ...
        _with_lifecycle_fields(
            {
                ...
                "wait_since_last_exit_min": wait_since_last_exit_min,
                "wait_since_last_entry_min": wait_since_last_entry_min,
                # ML training features (added for TradePredictor neural network)
                "unified_score": _unified_result.final_score if _unified_result else None,
                "unified_threshold": _unified_result.entry_threshold if _unified_result else None,
                "unified_tier": _unified_result.quality_tier if _unified_result else None,
                "rsi": round(_us_rsi, 2) if _us_rsi else None,
                "atr": round(float((selected_v4 or {}).get("atr_15m") or 0), 6),
                "volume_ratio": float((selected_v4 or {}).get("rvol") or 0),
                "trend_strength": float((selected_v4 or {}).get("adx_15m") or 0),
                "range_position": round(_us_rp, 4) if _us_rp else None,
                "sentiment_score": float((_us_sent_result or {}).get("score") or 0),
                "structure_bias": _structure_bias,
                "rr_ratio": round(_us_rr, 2) if _us_rr else None,
                "hour_utc": now.hour,
