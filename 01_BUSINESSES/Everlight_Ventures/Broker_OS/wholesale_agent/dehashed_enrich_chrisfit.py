...
def main() -> int:
    ...
    ready, queried, hits, total_emails, errors = [], 0, 0, 0, 0
    last_balance = None
    for i, lead in enumerate(batch, 1):
        ...
        if emails:
            ...
            ready.append({
                ...
                "dehashed_fields": emails[0],
            })
        flag = "HIT" if emails else ("ERR" if r.get("error") else "---")
        print(f"  [{i}/{len(batch)}] {flag} {name[:30]:30s} {emails[0]['email'] if emails else (r.get('error') or '')}")
    ...
    print("\n" + "=" * 66)
    print(f"RESULT: {hits}/{queried} owners returned a REAL email ({rate:.0f}% hit-rate)")
    print(f"        {total_emails} real emails | errors={errors} | credits left={last_balance}")
    print(f"        cost-per-reachable-owner = ~{(queried/hits):.1f} credits" if hits else "        no hits")
    print(f"wrote -> {OUT}")
