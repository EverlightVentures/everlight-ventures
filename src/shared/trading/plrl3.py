...
def _project_mr_after_add(
    *,
    ...
    for w in ("intraday", "overnight"):
        m = meas.get(w) or {}
        mm_cur = _num(m.get("maintenance_margin"))
        factor = float(m.get("mm_factor") or 1.0)
...
def compute_initial_contracts_plrl3(
    *,
    ...
    Interprets the user's sizing law as a maintenance-margin budget:
    ...
    # Use the worst-case per-contract maintenance burden.
    ...
    return max(0, initial), {"notes": notes, "budget_mm": budget_mm, "mm_per_contract": mm_per, "worst_mm_per_contract": worst}
...
def evaluate_plrl3(
    *,
    ...
    # Rescue: size increment determined by add_multipliers[step] (multiples of initial size).
    mult = None
    try:
    ...
    # Guardrails: must not project into danger.
    guard_notes = []
    ...
    if proj_o is not None:
        if window == "overnight":
            ...
            # pre-cutoff: require overnight MR to stay under a stricter guard if provided
...
# ── Score-10 upgrade: ATR-trail emergency exit ────────────────────────────────
#
# Billionaire rule: "Cut losers fast. Never average down on leverage."
# (Hive Mind: 25_analytics_auditor)
#
# Instead of adding contracts at margin triggers, this function checks whether
# the position should be EXITED based on ATR trailing stop price breach.
# Call this BEFORE evaluate_plrl3 to give the smarter exit priority.


def evaluate_atr_trail_exit(
    *,
    direction: str,
    entry_price: float,
    current_price: float,
    ...
    """ATR trailing stop exit signal.

    Returns {"exit": True/False, "trail_price": float, "reason": str}

    Logic:
      LONG:  trail = peak_price - atr_14 * multiplier
             exit if current_price <= trail
      SHORT: trail = trough_price + atr_14 * multiplier
             exit if current_price >= trail
    ...
    atr_multiplier : float
        Trail distance as multiple of ATR (default 2.5).
    peak_price : float | None
        Highest price seen since entry (LONG) or lowest (SHORT).
        If None, uses entry_price as the peak/trough.
    """
    if entry_price <= 0 or current_price <= 0 or atr_14 <= 0:
        return {"exit": False, "trail_price": None, "reason": "missing_inputs"}

    d = direction.upper() if direction else ""
    trail_distance = atr_14 * float(atr_multiplier)

    if "LONG" in d:
        peak = float(peak_price) if peak_price else entry_price
        ...
        if current_price <= trail_price:
            return {
                ...
                "peak": round(peak, 6),
                "reason": f"ATR_trail: price={current_price:.5f} <= trail={trail_price:.5f}",
            }
        return {
            "exit": False,
            "trail_price": round(trail_price, 6),
            "peak": round(peak, 6),
            "reason": f"holding: {current_price:.5f} > trail={trail_price:.5f}",
        }

    elif "SHORT" in d:
        trough = float(peak_price) if peak_price else entry_price
        trough = min(trough, current_price)  # update trough
    ...
    return {"exit": False, "trail_price": None, "reason": f"unknown_direction={direction}"}
