...
if _os_halt.environ.get("WHOLESALE_OUTBOUND_HALT", "").strip() in {"1", "true", "TRUE", "yes"}:
    ...
    print("[rex_batch_offers.py] WHOLESALE_OUTBOUND_HALT=1 -- refusing to run", file=_sys_halt.stderr)
...
try:
    ...
    print(f"[rex_batch_offers.py] eradication_gate unavailable: {_eg_err}", file=_sys_eg.stderr)
