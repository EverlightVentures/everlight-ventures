...
import { useApi, formatUSD, timeAgo } from "../hooks"
...
export default function Revenue() {
  const { data, error } = useApi("/api/django/revenue", 30000)
  ...
  if (!data && !error) {
    return (
      <div className="space-y-4 animate-pulse">
        ...
        <div className="card h-48 flex items-center justify-center"><div className="text-[10px] text-gray-600 tracking-widest">Loading Revenue...</div></div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-amber-400 tracking-wider">REVENUE</h1>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {STREAMS.map(s => {
            ...
            return (
              <div key={s.key} className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:border-white/[0.08] transition-colors">
                <div className={`w-10 h-10 rounded-lg ${s.bg} flex items-center justify-center font-bold text-sm ${s.color}`}>{s.icon}</div>
