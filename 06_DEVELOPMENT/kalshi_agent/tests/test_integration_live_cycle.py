...
from kalshi_agent.main import run_live_cycle
...
def _isolated_cycle():
    ...
    with patch("kalshi_agent.main._make_notifier", return_value=MagicMock()), \
         patch("kalshi_agent.main.gather_signals", return_value=sig), \
         patch("kalshi_agent.intelligence.SharedIntelligence", return_value=intel):
        yield
...
def test_live_cycle_maps_outcome_to_token_and_calls_backend(tmp_path, monkeypatch):
    ...
    with patch("kalshi_agent.main.PolymarketCLOB") as MockCLOB, \
         patch("kalshi_agent.agents.predictor.Predictor._llm_predict",
