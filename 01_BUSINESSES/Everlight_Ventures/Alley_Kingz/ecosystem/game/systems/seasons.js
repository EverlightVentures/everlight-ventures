...
(function (global) {
  'use strict';
  if (!global.AK_SYSTEMS) return;            // hub-only; node harness / battler page no-op
  ...
  var CHAPTERS = [
    { key:'junkyard', name:'JUNKYARD DYNASTY', glyph:'🦴', figure:'$BCARDD', faction:'Boneguard Crew',
    ...
    { key:'neonhowl', name:'NEON HOWL', glyph:'🌃', figure:'Jagged', faction:'Zoomie Syndicate',
    ...
    { key:'dogdays', name:'DOG DAYS', glyph:'☀️', figure:'Rosco', faction:'Leashbreak Tactix',
    ...
    { key:'bloodmoon', name:'BLOOD MOON', glyph:'🌑', figure:'Crown Foxhound', faction:'K9 Circuitry',
    ...
    { key:'frostbite', name:'FROSTBITE', glyph:'❄️', figure:'Stonejaw', faction:'Boneguard Crew',
    ...
    { key:'goldenleash', name:'GOLDEN LEASH', glyph:'👑', figure:'$BCARDD', faction:'Boneguard Crew',
  ...
  function dayKey(now) { return new Date(now || Date.now()).toISOString().slice(0, 10); }     // UTC YYYY-MM-DD
  ...
  // OPTIONAL integration hook -- index.html grantMatchRewards MAY call this; if it
  // never does, the module is fully functional via the daily check-in.
  function onMatchEnd(result) {
    try {
      var ctx = global.AK_CTX; if (!ctx || !ctx.econ) return;
  ...
  function renderHead() {
    ...
    if (HUB.glyphEl) HUB.glyphEl.textContent = ch.glyph;
    var h2 = document.getElementById('aksn-h2'), sub = document.getElementById('aksn-sub');
    if (h2) h2.textContent = ch.name;
    if (sub) sub.textContent = 'Week ' + weekOf() + ' of 6 -- ' + daysLeft() + ' day' + (daysLeft() === 1 ? '' : 's') + ' left -- led by ' + ch.figure;
    ...
    setKids(HUB.headEl, [ mk('span', { class:'aksn-marks', style:'color:' + ch.accent, text:'◆ ' + marksOf(ctx) + ' MARKS' }), btn ]);
  }
  ...
  function renderKeeper(ctx, b) {
    ...
    var ci = checkInStatus(ctx);
    var line = ch.name + '  --  Week ' + weekOf() + '/6, ' + daysLeft() + 'd left.  ◆ ' + marks + ' Marks. ' + ch.flavor;
