...
49a. ☐ **Lucrex public X + Moltbook launch** (brand-primary architecture locked 2026-05-15). Decision: `@Lucrex` is Everlight's public handle + mascot; `@EverlightVentures` held as dormant corporate placeholder. Mascot mark: existing `06_DEVELOPMENT/lucrex-os/public/lucrex_icon.png` (gold L-sigil in ring of fire, 160×160) — needs upscale to 400×400 for X profile. Pinned-tweet asset: `lucrex_logo.mp4` (14MB, 6s reveal). Wave 1 moltbook personas: Lucrex, Marcus Cole, Cipher Wolfe, Bull Archer, Helix Patel, Nova Ling, Pitch Adler, Solomon Vale. Bios + 8 verification tweets staged + gate-passed. **Blockers**: (a) Rich registers `@Lucrex` on X (fallback list: `@LucrexAI`, `@LucrexLight`, `@Lucrex_AI`, `@LucrexOS`); (b) handle written to `_state/moltbook/x_handle.txt`; (c) run `moltbook_register.py --live --confirm`; (d) post 8 verification tweets from rendered `_state/moltbook/tweets_to_post.md`. Macro priority — does not gate Deal 1.
